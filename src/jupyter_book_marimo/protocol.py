...
class MarimoCellRequest:
    ...
    def from_json(cls, value: Any) -> MarimoCellRequest:
        ...
        return cls(
            ...
            start_line=optional_int(payload.get("startLine")),
            end_line=optional_int(payload.get("endLine")),
        )
...
class MarimoPageMetadata:
    ...
    def from_json(cls, value: Any) -> MarimoPageMetadata:
        ...
        setup_cells = payload.get("setupCells") or []
...
class CompiledMarimoCell:
    ...
    def to_page_cell_json(self) -> JsonObject:
...
class MarimoPageRuntime:
    ...
    def from_json(cls, value: Any) -> MarimoPageRuntime:
        ...
        notebook_code = payload.get("notebookCode") or ""
        if not isinstance(app_id, str) or not app_id:
            raise TypeError("marimo app id must be a non-empty string")
        if not isinstance(runtime_cell_count, int) or isinstance(
            runtime_cell_count, bool
        ):
            raise TypeError("marimo runtime cell count must be an integer")
...
def page_cell_payload(
    page: CompiledMarimoPage,
    cell: CompiledMarimoCell,
) -> JsonObject:
    return {
        "protocolVersion": PAGE_PROTOCOL_VERSION,
        "app": page.app.to_json() if page.app else None,
        "cell": cell.to_page_cell_json(),
    }


def page_cell_reference_payload(
    page: CompiledMarimoPage,
    cell: CompiledMarimoCell,
) -> JsonObject:
    if page.app is None:
        return page_cell_payload(page, cell)
    return {
        "protocolVersion": PAGE_PROTOCOL_VERSION,
        "appId": page.app.id,
        "cell": cell.to_page_cell_json(),
...
def project_page_cell_payloads(
    page: CompiledMarimoPage,
    ...
    carrier_index = (
        next(
            (
                ...
                if cell.options.get("render", {}).get("include", True)
            ),
            None,
        )
    ...
    return tuple(
        None
        if not cell.options.get("render", {}).get("include", True)
        else (
            page_cell_payload(page, cell)
            if cell.index == carrier_index
            else page_cell_reference_payload(page, cell)
        )
        for cell in page.cells
    )
